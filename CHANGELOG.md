...
- **Retargeted the whole skill from Kotlin 2.4.10 to 2.4.20.** All EVIDENCE.md / guide.md permalinks re-pinned to `v2.4.20`; 61 of the 167 cited source paths changed across the 3,939 upstream commits, producing 64 line-drift citations and 2 moved/deleted paths, all re-anchored and re-verified (405 citations after the verification-driven additions, 0 dangling). Every changed cited file was read for plugin-facing API changes; the results below each gained a `## Kotlin 2.4.10 → 2.4.20` section in the topic's `CHANGES.md`.
