...
```
The `specific=[2.1.0...2.2.99]` block is empty (no override) for that range; the `default` block emits `pluginId` for all other versions. This matches the SKILL.md claim that pre-2.3 plugins must omit `pluginId`, while 2.3+ requires it.
