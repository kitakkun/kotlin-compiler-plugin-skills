...
The Kotlin Gradle plugin transitively pulls a stdlib matching the consumer's Kotlin version. Your plugin must not bundle stdlib (see `compiler-plugin-bootstrap`'s "Wrong artifact" gotcha). Always `compileOnly` for stdlib in the compiler plugin module, regardless of strategy.
...
- **Single-version foundation** → `compiler-plugin-bootstrap`
- **Distribution mechanics** → `gradle-plugin-integration`
- **CI testing across versions** → `compiler-plugin-testing`
...
- Cross-platform (JVM / JS / Native / Wasm) variance — see the platform-specific notes in `gradle-plugin-integration`
