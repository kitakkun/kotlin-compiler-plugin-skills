...
`MyGeneratedDeclarationKey` is the `GeneratedDeclarationKey` you used in the FIR-side generator (see `fir-declaration-generation-extension`). The IR origin carries the same key, letting both halves coordinate.
...
- **Lookup helpers and contexts** → `ir-plugincontext-usage`
- **Replace whole calls inside the body** → `ir-call-rewriting`
- **Generate the function declaration whose body you fill** → `fir-declaration-generation-extension` + this skill
- **Generate a brand-new class with brand-new functions** → `ir-synthetic-class-generation`
