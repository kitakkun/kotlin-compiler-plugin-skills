...
- **Pick which declarations to transform** → `fir-predicate-system`
- **Make annotated classes implement an interface, then make their methods open** → use both `fir-supertype-generation-extension` and this one
- **Generated declarations need explicit modifiers** → set them in the `*BuildingContext` helpers in `fir-declaration-generation-extension`; the status transformer doesn't run on plugin-generated declarations by default
