...
2. **Switch the whole toolchain from one property.** Drive the Kotlin version of the Gradle Kotlin plugin (KGP), `kotlin-compiler-embeddable`, and the test-framework artifacts from a single `kotlin.compiler` Gradle property so a build is `-Pkotlin.compiler=2.4.20` away from the other target. KGP itself is selected in `settings.gradle.kts` `pluginManagement` (it can't be set from a normal `dependencies` block):

   ```kotlin
   // settings.gradle.kts
   pluginManagement {
       val kotlinCompiler = providers.gradleProperty("kotlin.compiler").orElse("2.4.20")
...
3. **Prove both in CI.** Run the build matrix over the supported versions (`-Pkotlin.compiler=2.3.21` and `-Pkotlin.compiler=2.4.20`). See the "CI matrix" section below for the full workflow — Strategy 0 *is* that matrix applied to a single source set.
...
strategy:
  matrix:
    kotlin: [2.1.21, 2.2.20, 2.2.21, 2.3.0, 2.3.20, 2.3.21, 2.4.0, 2.4.10, 2.4.20]
steps:
...
```kotlin
val kotlinVersion = providers.gradleProperty("kotlin.compiler").orElse("2.4.20")
dependencies {
