...
The pre-migration API exposed a single `intercept`-like method that returned the new return type. Post-migration, the API is the **two-phase intercept/transform** contract documented in `SKILL.md`:
...
The 2.3.20 rename of `FirSimpleFunction` → `FirNamedFunction` (see CHANGES.md for `fir-sam-conversion-transformer-extension`) does **not** affect this extension's API surface — it works on `FirNamedFunctionSymbol` and `FirFunctionCall`, neither of which were renamed.
