...
`FirExtensionRegistrar.AVAILABLE_EXTENSIONS` has **18 entries** at v2.3.21 (verified against source `FirExtensionRegistrar.kt:29-50`). `FirReplSnippetResolveExtension::class` (entry 15) was added in the 2.1 cycle — confirmed absent at v2.0.21 and present at v2.1.20. The list is expected to grow over time as new extension points are added — when bumping to a newer Kotlin, re-check this list and add any new entries to the catalog table in the SKILL.md.
