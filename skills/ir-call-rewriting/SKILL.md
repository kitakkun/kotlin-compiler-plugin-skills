...
- **Set up the extension** → `compiler-plugin-bootstrap`
- **Look up the target and replacement symbols** → `ir-plugincontext-usage`
- **Inspect or modify the call's *body* (deeper than rewriting the call itself)** → `ir-body-modification`
- **Add a new function for the replacement** → `ir-synthetic-class-generation`
