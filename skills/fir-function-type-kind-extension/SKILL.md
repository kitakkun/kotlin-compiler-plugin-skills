...
Source: [`kotlin/compiler/fir/tree/src/org/jetbrains/kotlin/fir/extensions/FirFunctionTypeKindExtension.kt`](https://github.com/JetBrains/kotlin/blob/v2.3.21/compiler/fir/tree/src/org/jetbrains/kotlin/fir/extensions/FirFunctionTypeKindExtension.kt). Reference impl: [`kotlin/plugins/plugin-sandbox/src/org/jetbrains/kotlin/plugin/sandbox/fir/types/SandboxFunctionTypeKindExtension.kt`](https://github.com/JetBrains/kotlin/blob/v2.3.21/plugins/plugin-sandbox/src/org/jetbrains/kotlin/plugin/sandbox/fir/types/SandboxFunctionTypeKindExtension.kt) (declares `MyInlineable*` and `MyNotInlineable*` families). Built-in kinds template: [`kotlin/core/compiler.common/src/org/jetbrains/kotlin/builtins/functions/FunctionTypeKind.kt`](https://github.com/JetBrains/kotlin/blob/v2.3.21/core/compiler.common/src/org/jetbrains/kotlin/builtins/functions/FunctionTypeKind.kt) (`Function`, `SuspendFunction`, `KFunction`, `KSuspendFunction`).
...
**`IrFunction.parameters` and `IrCall.arguments` are unified at v2.3.21** (KT-68003). The deprecation-for-removal `@DeprecatedForRemovalCompilerApi` on `valueParameters` / `dispatchReceiver` / `extensionReceiver` / `getValueArgument` / `valueArgumentsCount` is **error-level** at v2.3.21 — you cannot `@Suppress("DEPRECATION")` past it. Use the unified `arguments` list (indexed slots) and `parameters` list (with `IrParameterKind`) instead. See `ir-call-rewriting` for the migration patterns.
...
- **For diagnostics on misuse of the new kind** (e.g. forbidding `@Stable` on suspend lambdas) → `fir-additional-checkers-extension`.
- **For backend handling** → an `IrGenerationExtension` is **mandatory** (not optional). Use `ir-call-rewriting` or `ir-body-modification` patterns to find and rewrite usages.
