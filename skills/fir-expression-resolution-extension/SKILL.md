...
Source: [`kotlin/compiler/fir/resolve/src/org/jetbrains/kotlin/fir/extensions/FirExpressionResolutionExtension.kt`](https://github.com/JetBrains/kotlin/blob/v2.3.21/compiler/fir/resolve/src/org/jetbrains/kotlin/fir/extensions/FirExpressionResolutionExtension.kt).
...
If your goal is "make `foo()` mean something different *after* it's been resolved", that's call rewriting (`ir-call-rewriting`), not expression resolution. Expression resolution influences *which* `foo` resolves; IR call rewriting changes *what `foo` does*.
...
Use the predicate system (`fir-predicate-system`) to filter by annotation rather than walking annotation lists per call.
...
- **Filter by annotation efficiently** → `fir-predicate-system`
- **Add the *function* the resolver should pick** → `fir-declaration-generation-extension` (declare it on the receiver type your extension makes implicit)
- **Rewrite the IrErrorCallExpression into a real IrGetValue** → `ir-body-modification` (mandatory companion — without this, runtime fails)
- **Replace the resolved call's bytecode** → `ir-call-rewriting`
- **Cache receiver-type lookups across calls** → `fir-session-components`
