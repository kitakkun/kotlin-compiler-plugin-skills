...
For frequent annotation checks, use the predicate system instead — see `fir-predicate-system`.
...
The IDE uses the same FIR plugin path *only* if your plugin is wired through `KotlinCompilerPluginSupportPlugin` — see `gradle-plugin-integration`. With raw `-Xplugin=`, the IDE will not run your checker.
...
then check `session.predicateBasedProvider.matches(predicate, declaration)` instead of walking annotations every time. Details in `fir-predicate-system`.
...
- **Synthesise declarations from annotations** → `fir-declaration-generation-extension`
- **Make annotated classes implement an interface** → `fir-supertype-generation-extension`
- **Make annotated functions `open`** → `fir-status-transformer-extension`
- **Need to filter by annotation efficiently** → `fir-predicate-system`
- **Test the diagnostic** → `compiler-plugin-testing` (FIR diagnostic tests)

## What this skill does NOT cover

- Predicate registration syntax in depth — see `fir-predicate-system`
