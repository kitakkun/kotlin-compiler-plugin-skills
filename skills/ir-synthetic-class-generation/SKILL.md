...
This skill assumes the FIR side is already wired (see `fir-declaration-generation-extension`). At the IR level you walk:
...
Once you've located the FIR-generated class, all the heavy lifting (fields, constructors, member bodies) follows the patterns in `ir-body-modification` and `ir-call-rewriting`. **The recommendation is: declare in FIR, fill bodies in IR.** Don't fight the framework.
...
- **Declare the class shape at frontend level** → `fir-declaration-generation-extension`
- **Add bodies to functions of the generated class** → `ir-body-modification`
- **Look up the symbols of supertypes / referenced classes** → `ir-plugincontext-usage`
